...
converted_users_df = (sales_df.dropDuplicates(["email"])
...
# TODO
conversions_df = (users_df.join(converted_users_df, on='email', how="outer")
                          .filter(col("email").isNotNull())
                          .withColumn("converted", when(col("converted").isNull(), lit(False)).otherwise(col("converted")))
                 )
...
# TODO
email_carts_df = conversions_df.join(carts_df, on='user_id', how="left_outer")
...
# TODO
abandoned_carts_df = (email_carts_df.filter("converted = False").filter(col("cart").isNotNull())
)
...
# TODO
abandoned_items_df = (abandoned_carts_df.withColumn("items", explode(col("cart")))
                                        .groupBy("items").count()
                     )
